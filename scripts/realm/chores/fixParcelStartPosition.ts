...
const parcelId = "18531";

function getInstallationsQuery() {
  return `
    {installations(first: 1000 where: {parcel: "${parcelId}", id_gt:"${id}" equipped: true}) {
      id
      x
      y
      type {
...
function getTilesQuery() {
  return `
    {tiles(first: 1000 skip: 0 where: {parcel: "${parcelId}", id_gt:"${id}" equipped: true}) {
      id
      x
      y
      type {
