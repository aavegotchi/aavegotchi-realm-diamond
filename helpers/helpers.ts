...
import { VerifyParams, Domain } from "./types";

import { currentTimestamp, address, sleep } from "./utils";
...
export async function deployProxyAdmin(owner: Signer): Promise<VerifyParams> {
...
export async function deployProxy(
  ...
  let Proxy = await hre.ethers.getContractFactory(
  ...
  return {
    contract: proxy,
    constructorArgs: [await address(logic), await address(admin), []],
  };
}

export async function getProxyContract(contract: Contract): Promise<Contract> {
  let Proxy = await hre.ethers.getContractFactory(
    "TransparentUpgradeableProxy"
  );
...
export async function deployAndInitializeVestingProxy(
  ...
  let tx = await alchemicaVesting
    .connect(owner)
    .initialize(beneficiary, start, decayFactor, revocable, {
      gasLimit: 1000000,
    });
  await tx.wait();
  return {
    contract: alchemicaVesting,
    constructorArgs: proxyVerify.constructorArgs,
  };
}
...
export async function deployVestingImplementation(
  ...
  let AlchemicaVesting = await hre.ethers.getContractFactory(
    "AlchemicaVesting"
  );
...
export async function deployGAXFactory(owner: Signer): Promise<VerifyParams> {
...
export async function deployGAXRouter(
  ...
  let contract = await contractFactory.deploy(
    await address(factory),
    WMATIC_ADDRESS
  );
  return {
    contract: contract,
    constructorArgs: [await address(factory), WMATIC_ADDRESS],
  };
}

export async function verify(verifyParams: VerifyParams[]): Promise<void> {
  console.log(
    "Sleeping for 2 minutes to let contracts be mined before verification."
  );
  await sleep(120000);
...
export async function deployVestingContracts(
  ...
  await sleep(10000);
  console.log(
    "Vesting Implementation: " + vestingImplementation.contract.address
  );
...
export async function deployAlchemica(
  ...
  await sleep(10000);
  console.log(
    "Alchemica Implementation: " + alchemicaImplementation.contract.address
  );
...
export async function releaseAndLP(
  ...
  const ghst = await hre.ethers.getContractAt(
  ...
  for (let i = 0; i < alchemicas.length; i++) {
    ...
    tx = await alchemica
      .connect(beneficiary)
      .approve(await address(router), alchIn[i]);
    await tx.wait();
    ...
    console.log(
      alchIn[i].toString() +
        " " +
        names[i] +
        " (wei) added to liquidity pool with " +
        ghstIn[i].toString() +
        " GHST (wei)"
    );
  }
