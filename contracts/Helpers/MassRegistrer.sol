...
contract MassRegistrer {
    ...
    function massRegistrerERC721Each(
        ...
        while (_tokenIDStart < _tokenIDEnd) {
            IERC721(_ERC721Contract).safeTransferFrom(msg.sender, _GBM, _tokenIDStart, "");
            GBMFacet(_GBM).registerAnAuctionToken(_ERC721Contract, _tokenIDStart, bytes4(keccak256("ERC721")), _initiator);
            _tokenIDStart++;
        }
    }
    ...
    function massRegistrerERC1155Default(
        ...
        while (_indexStart < _indexEnd) {
            GBMFacet(_GBM).registerAnAuctionToken(_ERC1155Contract, _tokenID, bytes4(keccak256("ERC1155")), address(0));
            _indexStart++;
        }
    }
    ...
    function massRegistrerERC1155Each(
        ...
        while (_indexStart < _indexEnd) {
            GBMFacet(_GBM).registerAnAuctionToken(_ERC1155Contract, _tokenID, bytes4(keccak256("ERC1155")), _initiator);
            _indexStart++;
        }
    }
}
