...
import {LibAppStorage, AppStorage, Whitelist} from "./AppStorage.sol";

library LibWhitelist {
  function getNewWhitelistId() internal view returns (uint256 whitelistId) {
    AppStorage storage s = LibAppStorage.diamondStorage();
    whitelistId = s.whitelists.length + 1; //whitelistId 0 is reserved for "none"
  }

  function _whitelistExists(uint256 whitelistId) internal view returns (bool exists) {
    AppStorage storage s = LibAppStorage.diamondStorage();
    exists = (s.whitelists.length >= whitelistId) && (whitelistId > 0);
  }

  function getWhitelistFromWhitelistId(uint256 whitelistId) internal view returns (Whitelist storage whitelist) {
    AppStorage storage s = LibAppStorage.diamondStorage();
    require(_whitelistExists(whitelistId), "WhitelistFacet: Whitelist not found");
    whitelist = s.whitelists[whitelistId - 1];
  }

  function checkWhitelistOwner(uint256 whitelistId) internal view returns (bool isOwner) {
