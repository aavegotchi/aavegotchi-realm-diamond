...
import "../TileDiamond/facets/TilesPolygonXGotchichainBridgeFacet.sol";

contract TilesBridgePolygonSide is ProxyONFT1155 {
  ...
  function _debitFrom(address _from, uint16, bytes memory, uint[] memory _tokenIds, uint[] memory _amounts) internal override {
    ...
    TilesPolygonXGotchichainBridgeFacet(address(token)).removeItemsFromOwner(_from, _tokenIds, _amounts);
  }
