...
contract GBM is IGBM, IERC1155TokenReceiver, IERC721TokenReceiver {
    ...
    function bid(uint256 _auctionID, uint256 _bidAmount, uint256 _highestBid) external override {
        ...
        require((_highestBid * (getAuctionBidDecimals(_auctionID)) + (getAuctionStepMin(_auctionID)/getAuctionBidDecimals(_auctionID))) >= _highestBid,
    ...
    function registerAnAuctionToken(address _contract, uint256 _tokenId, bytes4 _tokenKind, address _initiator) external{
    ...
    function modifyAnAuctionToken(address _contract, uint256 _tokenId, bytes4 _tokenKind, address _initiator, uint256 _1155Index, bool _rewrite) public {

        require(msg.sender == Ownable(_contract).owner(), "Only the owner of a contract can allow/disallow bidding");

        if(!_rewrite){
            require(auctionMapping[_contract][_tokenId][_1155Index] == 0, "The auction aleady exist for the specified token");
            require(_1155Index == 0, "_1155Index must be equal to 0 when registering new tokens");
        } else {
            require(auctionMapping[_contract][_tokenId][_1155Index] != 0, "The auction doesn't exist yet for the specified token");
        }
        ...
        if (_tokenKind == bytes4(keccak256("ERC721"))){
            ...
            require(
                msg.sender == Ownable(_contract).owner() || 
                eRC1155_tokensUnderAuction[_contract][_tokenId] < IERC1155(_contract).balanceOf(address(this), _tokenId),
                "registerAnAuctionToken:  the specified ERC-1155 token cannot be auctionned"
            );

            require(_1155Index <= eRC1155_tokensIndex[_contract][_tokenId], "The specified _1155Index have not been reached yet for this token");
    ...
    function getAuctionID(address _contract, uint256 _tokenID) external override view returns(uint256){
        return auctionMapping[_contract][_tokenID][0];
    }

    function getAuctionID(address _contract, uint256 _tokenID, uint256 _tokenIndex) external override view returns(uint256){
