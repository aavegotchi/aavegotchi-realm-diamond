...
contract GBM is IGBM, IERC1155TokenReceiver, IERC721TokenReceiver {
  ...
  mapping(address => mapping(uint256 => mapping(uint256 => uint256)))
    internal auctionMapping; // contractAddress => tokenId => TokenIndex => _auctionID
  ...
  mapping(address => mapping(uint256 => uint256))
    internal eRC1155_tokensUnderAuction; //Contract => TokenID => Amount being auctionned
  ...
  function bid(
    ...
    require(
      getAuctionEndTime(_auctionID) >= block.timestamp,
      "bid: Auction has already ended"
    );

    require(
      _bidAmount > _highestBid,
      "bid: _bidAmount must be higgher than _highestBid"
    );
    require(
      (_highestBid *
        (getAuctionBidDecimals(_auctionID)) +
        (getAuctionStepMin(_auctionID) / getAuctionBidDecimals(_auctionID))) >=
        _highestBid,
    ...
    if (
      ...
      block.timestamp + getHammerTimeDuration(_auctionID)
    ) {
      auction_endTime[_auctionID] =
        block.timestamp +
        getHammerTimeDuration(_auctionID);
    ...
    auction_dueIncentives[_auctionID] = calculateIncentives(
      _auctionID,
      _bidAmount
    );
  ...
  function claim(uint256 _auctionID) external override {
    ...
    require(
      collection_biddingAllowed[_ca],
      "claim: Claiming is currently not allowed"
    );
    require(
      getAuctionEndTime(_auctionID) < block.timestamp,
    ...
    if (tokenMapping[_auctionID].tokenKind == bytes4(keccak256("ERC721"))) {
      //0x73ad2146
      IERC721(_ca).safeTransferFrom(
        address(this),
        auction_highestBidder[_auctionID],
        _tid
      );
    } else if (
      tokenMapping[_auctionID].tokenKind == bytes4(keccak256("ERC1155"))
    ) {
      //0x973bb640
      IERC1155(_ca).safeTransferFrom(
        address(this),
        auction_highestBidder[_auctionID],
        _tid,
        1,
  ...
  function registerAnAuctionContract(address _contract, address _initiator)
    public
  {
    require(
    ...
    collection_startTime[_contract] = IGBMInitiator(_initiator).getStartTime(
      uint256(uint160(_contract))
    );
    collection_endTime[_contract] = IGBMInitiator(_initiator).getEndTime(
      uint256(uint160(_contract))
    );
    collection_hammerTimeDuration[_contract] = IGBMInitiator(_initiator)
    .getHammerTimeDuration(uint256(uint160(_contract)));
    collection_bidDecimals[_contract] = IGBMInitiator(_initiator)
    .getBidDecimals(uint256(uint160(_contract)));
    collection_stepMin[_contract] = IGBMInitiator(_initiator).getStepMin(
      uint256(uint160(_contract))
    );
    collection_incMin[_contract] = IGBMInitiator(_initiator).getIncMin(
      uint256(uint160(_contract))
    );
    collection_incMax[_contract] = IGBMInitiator(_initiator).getIncMax(
      uint256(uint160(_contract))
    );
    collection_bidMultiplier[_contract] = IGBMInitiator(_initiator)
    .getBidMultiplier(uint256(uint160(_contract)));
  }
  ...
  function setBiddingAllowed(address _contract, bool _value) external {
    require(
      msg.sender == Ownable(_contract).owner(),
      "Only the owner of a contract can allow/disallow bidding"
    );
  ...
  function modifyAnAuctionToken(
    ...
    require(
      _tokenKind == bytes4(keccak256("ERC721")) ||
        _tokenKind == bytes4(keccak256("ERC1155")),
    ...
    if (_tokenKind == bytes4(keccak256("ERC721"))) {
      require(
        msg.sender == Ownable(_tokenContract).owner() ||
          address(this) == IERC721(_tokenContract).ownerOf(_tokenId),
        "registerAnAuctionToken: the specified ERC-721 token cannot be auctioned"
      );

      auctionId = uint256(
        keccak256(abi.encodePacked(_tokenContract, _tokenId, _tokenKind))
      );
      auctionMapping[_tokenContract][_tokenId][0] = auctionId;
    } else {
      require(
        msg.sender == Ownable(_tokenContract).owner() ||
          eRC1155_tokensUnderAuction[_tokenContract][_tokenId] <
      ...
      require(
        _1155Index <= eRC1155_tokensIndex[_tokenContract][_tokenId],
        "The specified _1155Index have not been reached yet for this token"
      );

      auctionId = uint256(
        keccak256(
          abi.encodePacked(_tokenContract, _tokenId, _tokenKind, _1155Index)
        )
      );
    ...
    if (_initiator != address(0x0)) {
      auction_startTime[auctionId] = IGBMInitiator(_initiator).getStartTime(
        auctionId
      );
      auction_endTime[auctionId] = IGBMInitiator(_initiator).getEndTime(
        auctionId
      );
      auction_hammerTimeDuration[auctionId] = IGBMInitiator(_initiator)
      .getHammerTimeDuration(auctionId);
      auction_bidDecimals[auctionId] = IGBMInitiator(_initiator).getBidDecimals(
        auctionId
      );
      auction_stepMin[auctionId] = IGBMInitiator(_initiator).getStepMin(
        auctionId
      );
      auction_incMin[auctionId] = IGBMInitiator(_initiator).getIncMin(
        auctionId
      );
      auction_incMax[auctionId] = IGBMInitiator(_initiator).getIncMax(
        auctionId
      );
      auction_bidMultiplier[auctionId] = IGBMInitiator(_initiator)
      .getBidMultiplier(auctionId);
    }
  ...
  function getAuctionID(address _contract, uint256 _tokenID)
    external
    view
    override
    returns (uint256)
  ...
  function getTokenId(uint256 _auctionID)
    external
    view
    override
    returns (uint256)
    ...
    returns (uint256)
  {
    if (auction_hammerTimeDuration[_auctionID] != 0) {
      ...
      return
        collection_hammerTimeDuration[tokenMapping[_auctionID].contractAddress];
    }
  }
    ...
    returns (uint256)
  {
    ...
    uint256 baseBid = (auction_highestBid[_auctionID] *
